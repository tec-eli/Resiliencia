...
- **[Composing a Policy](./policy-composition)** — chain `Retry` with `CircuitBreaker` and `Timeout` in one
  explicit, validated order via `resiliencia-compose`.
- **[Testing with ManualClock](./testing-manual-clock)** — drive retries and timeouts deterministically in
  tests, with `resiliencia-test`'s `ManualClock`, instead of real `Thread.sleep` delays.
- **[Metrics with Micrometer](./metrics-micrometer)** — wire pattern and Policy events into Micrometer
  counters, gauges and timers with `resiliencia-metrics` and `resiliencia-micrometer`.
